...
export default function FlowMonitor({ tenantId, onCreateFlow, onEditFlow }) {
  ...
  const [extraCols, setExtraCols]         = useState(() => {
    try { return JSON.parse(localStorage.getItem('fms_extraCols') || '[]'); } catch { return []; }
  });
  ...
  return (
    <div className="w-full min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6">
        ...
        {showColConfig && (
          <div style={{ background:'var(--color-card)', border:'1px solid var(--color-border)', borderRadius:12, padding:'14px 16px', marginBottom:8 }}>
            <div style={{ fontSize:12, fontWeight:700, marginBottom:10 }}>📊 Extra columns to show in Live Orders table</div>
            {instances.length === 0 ? (
              ...
              return (
                <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                  {allCols.map(col => {
                    const checked = extraCols.includes(col);
                    return (
                      <label key={col} style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'4px 10px', borderRadius:8, cursor:'pointer', fontSize:11, fontWeight:600,
                        ...
                        <input type="checkbox" style={{ display:'none' }} checked={checked}
                          onChange={e => {
                          const next = e.target.checked ? [...extraCols, col] : extraCols.filter(c => c !== col);
                          setExtraCols(next);
                          localStorage.setItem('fms_extraCols', JSON.stringify(next));
                        }} />
                        {checked ? '✓ ' : ''}{col}
                      </label>
                    );
            ...
            <button onClick={() => { setExtraCols([]); localStorage.removeItem('fms_extraCols'); }} style={{ marginTop:8, fontSize:10, color:'var(--color-muted-foreground)', background:'none', border:'none', cursor:'pointer', textDecoration:'underline' }}>
              Clear all extra columns
            </button>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button onClick={() => setShowColConfig(c => !c)}
            className="ml-auto px-3 py-1.5 text-[11px] font-semibold border border-border rounded-lg hover:bg-muted transition flex items-center gap-1.5">
            ⚙️ {showColConfig ? 'Hide' : 'Configure'} Columns {extraCols.length > 0 ? `(${extraCols.length} extra)` : ''}
          </button>
        </div>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/40">
                    {['Order', 'Flow', ...extraCols, 'Current step', 'Assigned to', 'Deadline', 'Delay', 'Status', ''].map(h => (
