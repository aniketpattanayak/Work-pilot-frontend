...
export default function FlowBuilder({ tenantId, wizardConfig, onFlowSaved }) {
  ...
  const addField = (nodeId) => {
    const node = nodes.find(n => n.id === nodeId);
    updateNode(nodeId, { inputFields: [...(node.inputFields || []), { id: fid(), label: '', type: 'text', options: [], required: false, visibleToSteps: [] }] });
  };
                ...
                {(selNode.type === 'action' || selNode.type === 'input' || selNode.type === 'yesno') && (<>
                  ...
                  {(selNode.inputFields || []).map(field => (
                    <div key={field.id} style={{ border:'1px solid var(--color-border)', borderRadius:8, padding:8, marginBottom:6 }}>
                      ...
                      {/* Visible to future steps */}
                      {(() => {
                        const currentIdx = nodes.findIndex(n => n.id === selNode.id);
                        const futureNodes = nodes.slice(currentIdx + 1).filter(n => n.type !== 'end');
                        if (futureNodes.length === 0) return null;
                        return (
                          <div style={{ marginTop:6 }}>
                            <div style={{ fontSize:10, color:'var(--color-muted-foreground)', marginBottom:4, fontWeight:600 }}>
                              👁 Visible to steps (auto-shown when they open their task):
                            </div>
                            <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
                              {futureNodes.map(fn => {
                                const isChecked = (field.visibleToSteps || []).includes(fn.id);
                                return (
                                  <label key={fn.id} style={{ display:'flex', alignItems:'center', gap:4, fontSize:10,
                                    ...
                                    <input type="checkbox" style={{ display:'none' }}
                                      checked={isChecked}
                                      onChange={e => {
                                        const curr = field.visibleToSteps || [];
                                        updateField(selNode.id, field.id, {
                                          visibleToSteps: e.target.checked
                                            ? [...curr, fn.id]
                                            : curr.filter(id => id !== fn.id)
                                        });
                                      }} />
                                    {fn.name || fn.type}
                                  </label>
