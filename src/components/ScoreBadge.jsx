...
const ScoreBadge = ({ employeeId, minimalist = false }) => {
  ...
  useEffect(() => {
    const fetchScore = async () => {
      ...
      try {
        setLoading(true);
        // Using relative path for production flexibility
      ...
      <div className="flex items-center bg-card/80 backdrop-blur-xl border border-border rounded-2xl p-1.5 pr-4 gap-3 h-11 shadow-sm transition-all duration-500 hover:scale-105 active:scale-95 cursor-default group">
        ...
        <div className="relative w-8 h-8 flex-shrink-0 flex items-center justify-center">
          <svg className="w-full h-full -rotate-90 overflow-visible" viewBox="0 0 40 40">
            {/* The background track now uses the semantic border color */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-10 relative z-10 items-center">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col">
            <h3 className="text-foreground text-6xl font-black tracking-tighter leading-none flex items-baseline">
              {Math.round(numericScore)}<span className="text-3xl ml-1 opacity-20 dark:opacity-40">%</span>
            </h3>
              ...
              {earnedBadges.map((badge, i) => {
                ...
                return (
                  <div 
                    key={i} 
                    title={badge.name} 
                    className="p-3.5 rounded-2xl bg-card border border-border hover:border-primary transition-all hover:scale-110 hover:shadow-lg shrink-0" 
            ...
            <div className="flex items-center gap-6">
                ...
                <div className="flex flex-col min-w-0">
                    ...
                    <div className="flex items-baseline gap-2">
                        <span className="text-4xl font-black text-foreground leading-none tracking-tighter truncate">
      ...
      <div className="mt-12 flex flex-col md:flex-row gap-8 items-center justify-between relative z-10 border-t border-border/50 pt-8">
        <div className="w-full md:max-w-md space-y-4">
            ...
            <div className="w-full bg-background h-3.5 rounded-full border border-border p-1 overflow-hidden">
                <div 
                  className={`h-full rounded-full transition-all duration-[2000ms] ease-out ${isHighPerformer ? 'bg-emerald-500 shadow-[0_0_15px_#10b981]' : 'bg-amber-500 shadow-[0_0_15px_#f59e0b]'}`} 
        ...
        <div className={`
          ...
          ${isHighPerformer 
            ? 'bg-emerald-500/10 dark:bg-card border-emerald-500/20' 
            : 'bg-amber-500/10 dark:bg-card border-amber-500/20'
          }
        `}>
            {isHighPerformer 
