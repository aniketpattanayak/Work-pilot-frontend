...
const CHECKLIST_HEADERS = ['Checklist Title', 'Description', 'Assigned To', 'Frequency', 'Deadline', 'Items (semicolon separated)'];
const CHECKLIST_SAMPLE = [['Quality Check', 'Check product quality before dispatch', 'Aniket', 'Daily', '2026-07-15', 'Check dimensions;Check weight;Check packaging;Attach photos']];
...
// ── Main component ────────────────────────────────────────────────────────────
export default function BulkUpload({ tenantId, assignerId, onClose, onSuccess, mode = 'all' }) {
  const ALL_TABS = [
  ...
  // 1. Safely filter tabs and handle singular/plural mismatches ('checklists' -> 'checklist')
  const filteredTabs = mode === 'all'
    ? ALL_TABS
    : ALL_TABS.filter(t => t.id === mode || t.id + 's' === mode || t.id === mode + 's');

  // 2. Fallback to ALL_TABS if nothing matches, preventing the undefined crash
  const TABS = filteredTabs.length > 0 ? filteredTabs : ALL_TABS;
  ...
  const handleUpload = async () => {
    ...
    try {
      let res;
      // Fixed: Passed `assignerId` to all bulk-upload endpoints so the backend maps them correctly
      if (tab === 'employees') {
        res = await API.post('/tasks/bulk-employees', { tenantId, assignerId, employees: rows });
      } else if (tab === 'tasks') {
        res = await API.post('/tasks/bulk-tasks', { tenantId, assignerId, tasks: rows });
      } else {
        res = await API.post('/tasks/bulk-checklist', { tenantId, assignerId, checklists: rows });
      }

      setResult(res.data);
      setRows([]);

      // Update data in background but keep modal open so user can read Success/Failure details
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid var(--color-border)', padding: '0 24px' }}>
            {TABS.map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => { setTab(id); setRows([]); setResult(null); setError(''); }}
                style={{
                  display: 'flex', alignItems: 'center', gap: 6, padding: '12px 14px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 13,
              ...
              {result.results?.skipped?.length > 0 && (
                <div style={{ marginTop: 8 }}>
                  ...
                  {result.results.skipped.map((s, i) => (
                    <div key={i} style={{ fontSize: 12, color: '#344054' }}>• {s.name || s.title} — {s.reason}</div>
                  ))}
                </div>
              )}
